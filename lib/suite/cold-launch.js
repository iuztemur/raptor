...
ColdLaunch.prototype.setCoordinates = function() {
  ...
  var ordinalX = homescreenConfig.preferences['raptor.ordinalX'];
  var ordinalY = homescreenConfig.preferences['raptor.ordinalY'];
  var columnIncrement = homescreenConfig.preferences['raptor.columnIncrement'];
  var rowIncrement = homescreenConfig.preferences['raptor.rowIncrement'];
  ...
  var x = ordinalX + columnIncrement * column;
  var y = ordinalY + rowIncrement * row;

  this.appX = x * devicePixelRatio;
  this.appY = y * devicePixelRatio;
};
...
ColdLaunch.prototype.launch = function() {
  return this.deviceInput.tap(this.appX, this.appY, 1);
};
...
ColdLaunch.prototype.waitForHomescreen = function() {
  ...
  return new Promise(function(resolve) {
    ...
    runner.dispatcher.on('performanceentry', function listener(entry) {
      if (entry.context !== 'Homescreen' || entry.name !== 'fullyLoaded') {
        return;
      }
...
ColdLaunch.prototype.closeApp = function() {
  ...
  return Device.closeApp(this.appPid);
};
