...
ColdLaunch.prototype.waitForHomescreen = function() {
  ...
  return new Promise(function(resolve) {
    ...
    runner.dispatcher.on('performanceentry', function listener(entry) {
      debug('Received entry %s from %s', entry.name, entry.context);
...
ColdLaunch.prototype.prime = function() {
  ...
  debug('Pre-launching application');
  ...
  return this
    .waitForFullyLoaded()
    .then(function(entry) {
      return runner.device.util.kill(entry.pid);
    });
};
...
ColdLaunch.prototype.closeApp = function() {
  ...
  return this.device.util.kill(this.appPid);
};
